...
public final class ItemInterfaceMessage implements InputMessageListener {
    ...
    private void thirdSlot(Player player, MessageBuilder payload) {
        int interfaceId = payload.getShort(edgeville.net.ByteOrder.LITTLE);
        int itemId = payload.getShort(edgeville.net.ValueType.A);
        int slot = payload.getShort(edgeville.net.ValueType.A);
        if (interfaceId < 0 || slot < 0 || itemId < 0)
            return;
    ...
    private void equipItem(Player player, MessageBuilder payload) {
        ...
        int slot = payload.getShort(false, edgeville.net.ValueType.A);
        int interfaceId = payload.getShort(false, edgeville.net.ValueType.A);
        if (interfaceId < 0 || slot < 0 || itemId < 0)
            return;
        switch (itemId) {
    ...
    private void swapSlots(Player player, MessageBuilder payload) {
        int interfaceId = payload.getShort(edgeville.net.ValueType.A, edgeville.net.ByteOrder.LITTLE);
        payload.get(edgeville.net.ValueType.C);
        int fromSlot = payload.getShort(edgeville.net.ValueType.A, edgeville.net.ByteOrder.LITTLE);
        int toSlot = payload.getShort(edgeville.net.ByteOrder.LITTLE);
