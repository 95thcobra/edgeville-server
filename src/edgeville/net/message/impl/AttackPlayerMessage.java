...
import edgeville.game.character.newcombat.PlayerVersusEntity;
	...
	private boolean checkAttack(Player attacker, Player victim) {
		...
		if (!optional.isPresent()) {
			if (!Location.inWilderness(attacker) || !Location.inWilderness(victim)) {
				attacker.getMessages().sendMessage("Both you and " + victim.getFormatUsername() + " need to be in the wilderness" + " to fight!");
				return false;
			}
			...
			if (!attacker.getCombatBuilder().isBeingAttacked() || attacker.getCombatBuilder().isBeingAttacked() && attacker.getCombatBuilder().getLastAttacker() != victim && Location.inMultiCombat(attacker)) {
